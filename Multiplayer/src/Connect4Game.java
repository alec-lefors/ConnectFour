...
public class Connect4Game extends JFrame implements ActionListener
{
	...
	public static void main(String[] args) throws ClassNotFoundException, InstantiationException, IllegalAccessException, UnsupportedLookAndFeelException
	{
		...
		JPanel startMultiGame = new JPanel();
		JPanel joinMultiGame = new JPanel();
		
		JTextField pickPortNum = new JTextField(5);
		JTextField ipAddy = new JTextField(20);
		JTextField portNum = new JTextField(5);
		
		startMultiGame.add(new JLabel("Port:"));
		startMultiGame.add(pickPortNum);
		joinMultiGame.add(new JLabel("IP Address:"));
		joinMultiGame.add(ipAddy);
		joinMultiGame.add(new JLabel("Port:"));
		joinMultiGame.add(portNum);
		...
		JMenu multiplayerTab = new JMenu("Multiplayer");
		...
		JMenuItem startMultiplayerGame = new JMenuItem("Start game...", null);
		JMenuItem joinMultiplayerGame = new JMenuItem("Join game...", null);
		...
		startMultiplayerGame.setToolTipText("Starts a new multiplayer game.");
		joinMultiplayerGame.setToolTipText("Join a game session.");
		
		// Adds items to each tab
		file.add(newGame);
		file.add(exitGame);
		experimental.add(changeColor);
		multiplayerTab.add(startMultiplayerGame);
		multiplayerTab.add(joinMultiplayerGame);
		changeColor.add(changeColorDefault);
		changeColor.add(changeColorBlack);
		menuBar.add(file);
		menuBar.add(experimental);
		menuBar.add(multiplayerTab);
		...
		changeColorDefault.addActionListener((ActionEvent e) ->
		{
			imagePanel.setBackground(background);
		});
		changeColorBlack.addActionListener((ActionEvent e) ->
		{
			imagePanel.setBackground(new Color(20, 20, 20));
		});
		startMultiplayerGame.addActionListener((ActionEvent e) ->
		{
			int result = JOptionPane.showConfirmDialog(app, startMultiGame, "Connect Four Multiplayer", JOptionPane.OK_CANCEL_OPTION);
			if(result == JOptionPane.OK_OPTION)
			{
				System.out.println(pickPortNum.getText());
				String[] arguments = {pickPortNum.getText()};
				try {
					Connect4Server.main(arguments);
				} catch (IOException e1) {
					e1.printStackTrace();
				}
			}
		});
		joinMultiplayerGame.addActionListener((ActionEvent e) ->
		{
			int result = JOptionPane.showConfirmDialog(app, joinMultiGame, "Connect Four Multiplayer", JOptionPane.OK_CANCEL_OPTION);
			if(result == JOptionPane.OK_OPTION)
			{
				System.out.println(ipAddy.getText());
				System.out.println(portNum.getText());
				String[] arguments = {ipAddy.getText(), portNum.getText()};
				try {
					Connect4Client.main(arguments);
				} catch (IOException e1) {
					e1.printStackTrace();
				}
			}
		...
		for(int i = 0; i < 7; i++)
		{
			...
			hovLabels[0].setPreferredSize(new Dimension(55, 55));
		}
